...
func (p *tarballImageProvider) Provide(ctx context.Context, path string, platform *image.Platform) (*image.Image, error) {
	...
	return NewDirectoryProvider(p.tmpDirGen).Provide(ctx, tempDir, platform)
}
